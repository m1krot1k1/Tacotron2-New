...
# 🔥 ИСПРАВЛЕНИЕ: Безопасные импорты для training_utils
try:
...
def prepare_dataloaders(hparams):
    ...
    # 🔥 ИСПРАВЛЕНИЕ: Разделяем логику создания DataLoader в зависимости от использования batch_sampler
    if use_bucket_batching and BucketBatchSampler is not None:
        # Для bucket batching используем только batch_sampler
        ...
        # Для стандартного DataLoader используем batch_size, shuffle, sampler
        train_loader = DataLoader(
            trainset,
            num_workers=1,
            shuffle=shuffle,
...
def train(
    ...
    if is_main_node:
        try:
            ...
            print("🎯 Alignment Diagnostics активирован")
        except Exception as e:
            print(f"⚠️ Не удалось инициализировать Alignment Diagnostics: {e}")
    restart_attempts = 0
            ...
            for i, batch in enumerate(train_loader):
                ...
                if hparams.fp16_run and use_native_amp:
                    ...
                    if alignment_diagnostics is not None and y_pred is not None and len(y_pred) >= 4 and y_pred[3] is not None:
                        try:
                            # Анализируем alignment каждые 100 шагов
                            if iteration % 100 == 0:
                                attention_matrix = y_pred[3].detach().cpu().numpy()
                                if attention_matrix.ndim == 3:  # [batch, time, mel_time]
                                    # Берем среднее по batch
                                ...
                                # Выполняем диагностику alignment
                                alignment_metrics = alignment_diagnostics.analyze_alignment_matrix(
                                    attention_matrix, 
                                    step=iteration,
                                    text_length=attention_matrix.shape[1],
                                    audio_length=attention_matrix.shape[0]
                                )
                                ...
                                if alignment_metrics['overall_score'] < 0.3:
                                    ...
                                    if debug_reporter:
                                        debug_reporter.add_warning(f"Poor alignment quality: {alignment_metrics['overall_score']:.3f}")
                                
                                # Логируем в MLflow
                                if MLFLOW_AVAILABLE:
                                    try:
                                        mlflow.log_metrics({
                                            "alignment.overall_score": alignment_metrics['overall_score'],
                                            "alignment.diagonal_score": alignment_metrics['diagnostics']['diagonal_score'],
                                            "alignment.monotonic_score": alignment_metrics['diagnostics']['monotonic_score'],
                                            "alignment.focus_score": alignment_metrics['diagnostics']['focus_score']
                                        }, step=iteration)
                                    except Exception as e:
                                        print(f"⚠️ Ошибка логирования alignment метрик: {e}")
                                
                                # Отправляем Telegram уведомление при критических проблемах
                                if telegram_monitor and alignment_metrics['overall_score'] < 0.2:
                                    try:
                                        critical_message = f"🚨 **КРИТИЧЕСКАЯ ПРОБЛЕМА ALIGNMENT!**\n\n"
                                        critical_message += f"📊 **Общий score:** {alignment_metrics['overall_score']:.3f}\n"
                                        critical_message += f"🎯 **Диагональность:** {alignment_metrics['diagnostics']['diagonal_score']:.3f}\n"
                                        critical_message += f"📈 **Монотонность:** {alignment_metrics['diagnostics']['monotonic_score']:.3f}\n"
                                        critical_message += f"🔍 **Фокусировка:** {alignment_metrics['diagnostics']['focus_score']:.3f}\n"
                                        critical_message += f"\n🛠️ **Рекомендации:**\n"
                                        for rec in alignment_metrics['recommendations'][:3]:
                                            critical_message += f"  • {rec}\n"
                            ...
                            print(f"⚠️ Ошибка alignment diagnostics: {e}")
                ...
                if loss is not None:
                    ...
                    if debug_reporter:
                        debug_reporter.add_metric("gradient_norm", grad_norm)
