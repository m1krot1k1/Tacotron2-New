    ...
    while true; do
        ...
        case $main_choice in
            ...
            3)
                ...
                echo "----------------------------------------"
                echo "1. 🤖 ПОЛНОСТЬЮ АВТОМАТИЧЕСКИЙ РЕЖИМ (рекомендуется)"
                echo "   ↳ Сначала оптимизация → затем обучение с лучшими параметрами"
                echo "2. 🔍 Только оптимизация гиперпараметров"
                echo "3. 🚀 Только обучение с лучшими параметрами (если уже есть оптимизация)"
                echo "4. 📊 Мониторинг существующего обучения"
                echo "----------------------------------------"
                read -p "Выберите режим [1-4]: " training_mode
                
                case $training_mode in
                    1)
                        echo ""
                        echo "🤖 ПОЛНОСТЬЮ АВТОМАТИЧЕСКИЙ РЕЖИМ"
                        echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
                        echo "🎯 ЧТО БУДЕТ ПРОИСХОДИТЬ:"
                        echo "  1️⃣ Оптимизация гиперпараметров (поиск лучших настроек)"
                        echo "  2️⃣ Автоматическое обучение с найденными лучшими параметрами"
                        echo "  3️⃣ Дополнительная оптимизация при необходимости"
                        echo "  4️⃣ Финальное обучение высококачественной модели"
                        echo ""
                        echo "🔧 НАСТРОЙКИ:"
                        echo "• Все TTS улучшения активны"
                        echo "• Умные критерии качества"
                        echo "• Adaptive Advisor включен"
                        echo "• Композитные метрики"
                        echo "• Полностью автоматизированный процесс"
                        echo ""
                        read -p "Количество trials для оптимизации [15]: " n_trials
                        n_trials=${n_trials:-15}
                        
                        echo ""
                        echo "🚀 Запуск автоматического обучения..."
                        echo "Мониторинг: TensorBoard, MLflow UI, Optuna Dashboard."
                        echo ""
                        read -p "Нажмите Enter для запуска автоматического режима..."
                        
                        run_command "python smart_tuner_main.py --mode auto --trials $n_trials"
                        ;;
                    2)
                        echo ""
                        echo "🔍 РЕЖИМ: Только оптимизация гиперпараметров"
                        echo "• Автоматически найдет лучшие параметры"
                        echo "• Запустит множественные trials с Optuna"
                        echo "• Все TTS улучшения интегрированы"
                        echo "• Рекомендуемое количество trials: 10-30"
                        echo ""
                        read -p "Количество trials для оптимизации [20]: " n_trials
                        n_trials=${n_trials:-20}
                        
                        echo ""
                        echo "Процесс оптимизации будет автоматически управляться Smart Tuner V2."
                        echo "Все улучшения (Adaptive Advisor, композитные метрики, фазовое обучение) активны."
                        echo "Мониторинг: TensorBoard, MLflow UI, Optuna Dashboard."
                        echo ""
                        read -p "Нажмите Enter для запуска оптимизации..."
                        
                        run_command "python smart_tuner_main.py --mode optimize --trials $n_trials"
                        ;;
                    3)
                        echo ""
                        echo "🚀 РЕЖИМ: Адаптивное обучение с автоматической оптимизацией"
                        echo "• Автоматически оптимизирует параметры при необходимости"
                        echo "• Перезапускает обучение с улучшенными параметрами"
                        echo "• Все TTS улучшения интегрированы"
                        echo "• Adaptive Advisor активен"
                        echo "• Композитные TTS метрики включены"
                        echo "• До 3 итераций для достижения качества"
                        echo ""
                        echo "Процесс обучения будет автоматически управляться Smart Tuner V2."
                        echo "Система адаптивно реагирует на проблемы, оптимизирует параметры и"
                        echo "при необходимости перезапускает обучение с улучшенными настройками."
                        echo ""
                        read -p "Нажмите Enter для запуска адаптивного обучения..."
                        
                        run_command "python smart_tuner_main.py --mode train"
                        ;;
                    4)
                        echo ""
                        echo "📊 РЕЖИМ: Мониторинг"
                        echo "• Мониторинг запущен"
                        echo "• TensorBoard: http://$(hostname -I | awk '{print $1}'):5001"
                        echo "• MLflow UI: http://$(hostname -I | awk '{print $1}'):5000"
                        echo "• Optuna Dashboard: http://$(hostname -I | awk '{print $1}'):5002"
                        echo "• TTS Demo: http://$(hostname -I | awk '{print $1}'):5003"
                        echo ""
                        
                        run_command "python smart_tuner_main.py --mode monitor"
                        ;;
                    *)
                        echo "❌ Неверный выбор. Запускаем автоматический режим по умолчанию..."
                        run_command "python smart_tuner_main.py --mode auto --trials 15"
                        ;;
                esac
