#!/usr/bin/env python3
"""
Smart Tuner V2 - Enhanced Main Entry Point
🚀 АВТОМАТИЗИРОВАННАЯ СИСТЕМА ОБУЧЕНИЯ TACOTRON2 С МАКСИМАЛЬНЫМИ ВОЗМОЖНОСТЯМИ

Интегрирует все улучшения из enhanced_training_main.py:
...
- 🆕 РАСШИРЕННОЕ ЛОГИРОВАНИЕ (10+ метрик в TensorBoard/MLflow)
...
# 🚀 ИСПОЛЬЗУЕМ УЛУЧШЕННУЮ ВЕРСИЮ с интеграцией enhanced_training_main.py
from smart_tuner.trainer_wrapper import EnhancedTrainerWrapper as TrainerWrapper
...
class SmartTunerMain:
    ...
    def run_automatic_mode(self, n_trials: int = 15) -> Dict[str, Any]:
        """
        🤖 ПОЛНОСТЬЮ АВТОМАТИЧЕСКИЙ РЕЖИМ С РАСШИРЕННЫМИ ВОЗМОЖНОСТЯМИ
        Сначала оптимизация гиперпараметров, затем ENHANCED обучение с лучшими параметрами
        Интегрирует все возможности из enhanced_training_main.py
        """
        self.logger.info("🤖 Запуск ENHANCED автоматического режима TTS обучения")
        self.logger.info("=" * 80)
        ...
        try:
            ...
            if optimization_results and optimization_results.get('best_parameters'):
                ...
                # ЭТАП 2: ENHANCED обучение с лучшими параметрами
                self.logger.info("🚀 ЭТАП 2/2: ENHANCED обучение с лучшими параметрами")
                self.logger.info("📊 Включает: расширенное логирование, фазовое обучение, современные техники")
                self.logger.info("=" * 50)
