...
| **AlertManager** | `alert_manager.py` | Отправляет уведомления в Slack/Discord/Email о ключевых событиях. |
| **ModelRegistry** | `registry.py` | Регистрирует лучшие версии моделей в MLflow Model Registry. |
...
4.  **Перезапуск "на лету":**
    ...
    *   `TrainerWrapper` немедленно перезапускает `train.py`, указывая путь к последнему чекпоинту (`--warm-start` или `--checkpoint_path`) и передавая новый набор гиперпараметров.
